...
public class TinfoilAI {
    ...
    private static func makeVerifier(
        ...
        if let pinnedMeasurement, let enclaveURL {
            return SecureClient(enclaveURL: enclaveURL, pinnedMeasurement: pinnedMeasurement)
        }
        if let enclaveURL {
            return SecureClient(
    ...
    public static func create(
        ...
        do {
            ...
            // selected domain and refreshes that domain's bundle. A pinned
            // measurement is bound to one enclave, so it always re-verifies
            // the configured enclave.
            let pinnedRefreshEnclaveURL = baseURL == nil || pinnedMeasurement != nil ? enclaveURL : nil
